 ...
 */
   ...
   <!-- 
    ...
    <c:if test="${roomVO!=null}">
      <!-- 객실 뽑을 for문 -->
      <c:forEach var="hotelroomvo" items="${roomVO}">
      ...
      <div  hidden="${historyvo.checkout}"></div> --%>
        ...
        <c:if test="${roomVO.imgList!=null}">
          ...
          <div class="text">
            ...
            <span class="d-block mb-4"><span class="text-uppercase letter-spacing-2">주중가 : </span><span class="display-4 text-primary" >${roomVO.weekPrice} ~</span> <span class="text-uppercase letter-spacing-2">/ per night</span> </span>
            <span class="d-block mb-4"><span class="text-uppercase letter-spacing-2">주말가 : </span><span class="display-4 text-primary" >${roomVO.weekenPrice} ~</span> <span class="text-uppercase letter-spacing-2">/ per night</span> </span>
            <h5 class="mb-4">침대갯수 : ${roomVO.roomType.substring(1,2)}개 / 수용인원 : ${roomVO.roomType.substring(2)}명</h5>
            <p class="mb-4">${roomVO.roomInfo}</p>
            <p><a href="#" class="btn btn-primary text-white" onclick="goView('${roomVO.productId}','${roomVO.productName}','${roomVO.roomType.substring(1,2)}','${roomVO.roomType.substring(2)}','${roomVO.weekPrice}')" style="cursor: pointer;">예약하기</a></p>
          </div>
        </div>
        </c:forEach>
      <!-- 객실 뽑을 for문 -->
...
<!-- .tab-pane -->
      ...
      <input type="hidden" name="productName" value="${roomVO.productName}"/>
