...
<script type="text/javascript">
   ...
   $(document).ready(function(){
     ...
     /*  $.ajax({ // 서브서치 부분 ajax처리
       ...
       largeCategoryCodeList = '${searchvo.largeCategoryCode}'.split(',');
       lontionList = '${searchvo.lontion}'.split(',');
       pontionList = '${searchvo.pontion}'.split(',');
            ...
            if(json.length > 0) { 
            	...
            	$.each(json, function(entryIndex, item){
            		...
            		if(	minPrice != null && maxPrice != null) {
            			$("#minPrice").val(Number('${searchvo.minPrice}'));
        	 			$("#maxPrice").val(Number('${searchvo.maxPrice}'));

        	 			$("#price-range").slider({
         	                range: true,
          	                min: Number(minPrice),
         	                max: Number(maxPrice),
         	                values: [Number('${searchvo.minPrice}'), Number('${searchvo.maxPrice}')],
            		...
            		if(largeCategoryCode != null && largeCategoryCode.length > 0) {
						bool = false;            			
            			if(largeCategoryCodeList!=null){
            				 for ( var i in largeCategoryCode ) {
            				 	if(largeCategoryCodeList[i]==largeCategoryCode){
            				 		bool=true;
            				 		break;
            				 	}
            				 }//for문
            					 ...
            					 if(bool)
            					 	largeCategoryCodeDiv += '" checked>';
           					 	 else
            					 	largeCategoryCodeDiv += '" >';
            					 largeCategoryCodeDiv += '   <label for="'+largeCategoryCode+'" >'+largeCategoryName+'</label>'+
				                    					 '</li>';
            					 
            			} else {//largeCategoryCodeList있을때 
		           			largeCategoryCodeDiv += '<li class="">'+
								                    '       <input type="checkbox" name="largeCategoryCode" value="'+largeCategoryCode+'" id="'+largeCategoryCode+'" >'+
            			...
            			if(lontion != null && lontion.length > 0) {
            				bool = false;            			
                			if(lontionList!=null){
                				 for ( var i in lontion ) {
                				 	if(lontionList[i]==lontion){
                				 		bool=true;
                				 		break;
                				 	}
                				 }//for문
                					 ...
                					 if(bool)
                						 lontionDiv += '" checked>';
               					 		...
               					 		lontionDiv += '" >';
                					 	lontionDiv += '   <label for="l'+lontion+'" >'+lontion+'</label>'+
			                   						  '</li>';
            		...
            		if(pontion != null && pontion.length > 0) {
            			bool = false;            			
            			if(pontionList!=null){
            				 for ( var i in pontion ) {
            				 	if(pontionList[i]==pontion){
            				 		bool=true;
            				 		break;
            				 	}
            				 }//for문
            					 ...
            					 if(bool)
            						 pontionDiv += '" checked>';
           					 		...
           					 		pontionDiv += '" >';
            					 	pontionDiv +=  '   <label for="p'+pontion+'" >'+pontion+'</label>'+
							          			   '</li>';
	  ...
	  $("#hotelName").keyup(function(){
		...
		// 별점범위 슬라이더바
		$("#minStar").val(Number('${searchvo.minStar}'));
		$("#maxStar").val(Number('${searchvo.maxStar}'));
		
		$("#star-range").slider({
                range: true,
                min: 0,
                max: 10,
                values: [Number('${searchvo.minStar}')*2, Number('${searchvo.maxStar}')*2],
                slide: function(event, ui) {
					...
					<div class="row">
								...
								<div class="col-md-6 mb-3 mb-md-0">
									...
									<div class="field-icon-wrap">
										...
										<select name="children" id="children" class="form-control">
											<c:if
												test="${searchvo.children!=null and searchvo.children!=''}">
											...
											<c:if
												test="${searchvo.children==null or searchvo.children==''}">
						...
						<div class="col-md-6 mb-3 mb-lg-0 col-lg-2">
							...
							<div class="field-icon-wrap">
								...
								<input value="${searchvo.checkin_date}" type="text"
						...
						<div class="col-md-6 mb-3 mb-lg-0 col-lg-2">
							...
							<div class="field-icon-wrap">
								...
								<input value="${searchvo.checkout_date}" type="text"
						...
						<div class="col-md-6 col-lg-2 align-self-end">
							<button id="btnSubSearch"
								class="btn btn-primary btn-block text-white">검색</button>
						</div>
					</div>
				</form>
			</div>
		...
		<div class="row" id="sortDiv">
			<select name="sort" id="sort" class="form-control col-md-3 col-sm-3"
				...
				<c:if test="${searchvo.sort!=null}">
					<c:if
						test="${searchvo.sort=='largecategoryontioncode' or searchvo.sort==''}">
				...
				<form name="subSearchFrm">
					<div id="subSearchInfo">
						...
						<input type="hidden" name="searchWord"
							value="${searchvo.searchWord}"> <input type="hidden"
							name="sort" value="${searchvo.sort}"> <input
							type="hidden" name="checkout_date"
							value="${searchvo.checkout_date}"> <input type="hidden"
							name="checkin_date" value="${searchvo.checkin_date}"> <input
							type="hidden" name="children" value="${searchvo.children}">
						<input type="hidden" name="adult" value="${searchvo.adult}">
					...
					<fieldset>
						<h4 aria-expanded="true" role="button" tabindex="0"
							style="margin-bottom: 15px;">옵션으로 검색</h4>
						<div>
							<input type="text" id="hotelName" class="form-control"
								name="hotelName" value="${searchvo.hotelName}"
								placeholder="숙박명 " autocomplete="off">
							<div id="displayList2"></div>
							<button id="optionSearch"
								class="btn btn-primary btn-block text-white"
								style="margin-top: 8px; margin-bottom: 16px;">숙박명으로
								검색하기</button>
						</div>
					</fieldset>
						...
						<fieldset>
							...
							<div id="filter-price-contents" class="filter-contents">
								...
								<input type="text" id="priceAmount" readonly
									style="border: 0; color: #f6931f; font-weight: bold; background: transparent;">
							</div>
						</fieldset>
						<fieldset>
							<h4 aria-expanded="true" role="button" tabindex="0"
							...
							<div id="filter-guest-rating-contents" class="filter-contents">
								...
								<input type="text" id="starAmount" readonly
									style="border: 0; color: #f6931f; font-weight: bold; background: transparent;">
							</div>
						</fieldset>
						<fieldset>
