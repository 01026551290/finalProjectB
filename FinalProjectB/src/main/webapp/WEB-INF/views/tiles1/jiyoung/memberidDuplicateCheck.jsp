...
<script type="text/javascript">
	...
	function goCheck() {
		...
		if(userid == "") {
			...
			frm.method = "post";
			frm.action = "memberidDuplicateCheck.go";
			frm.submit();
		}
	...
	*/	
	...
	//  또는
	    $(opener.document).find("#MEMBERID").val(userid);
...
<span style="font-size: 10pt; font-weight: bold;" >${method}</span>
		...
		<table style="width: 95%; height: 100px;">
			<tr>
				<td style="text-align: center;">
					...
					<input type="text" name="memberId" id="MEMBERID" size="20" maxlength="20" class="box" /><br style="line-height: 300%;"/> <!-- id는 css에서 쓰고, name은 데이터베이스에서 쓴다. -->
 ...
 <!--   전송방식이 POST 이라면 DB에 들어가서 입력한 ID가 사용가능한 ID인지 
	...
	<c:if test="${method =='POST'}">
		...
		<c:if test="${isUsememberId ==true}">

			<div align="center">
			    <br style="line-height: 400%"/>
				ID로[<span style="color: red; font-weight: bold;">${memberId}</span>] 사용가능합니다.
				<br/><br/><br/>
				<button type="button" class="box" onclick="setUserID('${memberId}')">닫기</button>
			</div>
			</c:if>
		
		<%-- ID가 중복되어 사용불가능한 경우  --%>
		<c:if test="${isUsememberId ==false}">
			<div align="center">
			  [<span style="color: red; font-weight: bold;">${memberId}</span>] 는 이미 사용중입니다.
			  <br/><br/>
			  <form name="frmIdcheck">
				<table style="width: 95%; height: 100px;">
					<tr>
						<td style="text-align: center;">
						          아이디를 입력하세요  <br style="line-height: 200%;"/>							    					     
							<input type="text" name="MEMBERID" id="MEMBERID" size="20" maxlength="20" class="box" /><br style="line-height: 300%;"/>  
							<span id="error" style="color: red; font-size: 12pt; font-weight: bold;">아이디를 입력하세요!</span><br/> 
							<button type="button" class="box" onclick="goCheck();" >확인</button>     
						</td>
					</tr>
