package com.spring.god.jinsoo.websocket;
...
import com.spring.god.jinsoo.model.MessageVO;
...
public class WebsocketEchoHandler extends TextWebSocketHandler {
        ...
        */
	    ...
	    protected void handleTextMessage(WebSocketSession wsession, TextMessage message) 
            ...
            */
	    	...
	    	JinsooadminVO adminvo = (JinsooadminVO)map.get("adminvo");  // "loginuser" 은 HttpSession에 저장된 키 값이다. 
	    	
	    	System.out.println("====> 웹채팅확인용 : 로그인ID : " + adminvo.getName());
	    	// ====> 웹채팅확인용 : 로그인ID : seoyh
	            ...
	            if (messageVO.getType().equals("all")) { // 채팅할 대상이 "전체" 일 경우 
	                if (!wsession.getId().equals(webSocketSession.getId())) {  // 메시지를 자기자신을 뺀 나머지 모든 사용자들에게 메시지를 보냄.
	                    webSocketSession.sendMessage(
	                            new TextMessage(wsession.getRemoteAddress().getAddress().getHostAddress() +" [" +adminvo.getName()+ "]" + " ▶ " + messageVO.getMessage()));  
	                }
	                ...
	                if (messageVO.getTo().equals(hostAddress)) {
	                    webSocketSession.sendMessage(
	                            new TextMessage(
	                                    ...
	                                    + wsession.getRemoteAddress().getAddress().getHostAddress() +" [" +adminvo.getName()+ "]" + "▶ " + messageVO.getMessage()
	                                    + "</span>") );
        ...
        */
	    ...
	    public void afterConnectionClosed(WebSocketSession wsession, CloseStatus status) 
	    	...
	    	Map<String, Object> map = wsession.getAttributes();
	    	JinsooadminVO adminvo  = (JinsooadminVO)map.get("adminvo");
	    	
	    	connectedUsers.remove(wsession);
	   	 
	        for (WebSocketSession webSocketSession : connectedUsers) {
	            if (!wsession.getId().equals(webSocketSession.getId())) { // 메시지를 자기자신을 뺀 나머지 모든 사용자들에게 메시지를 보냄.
	                webSocketSession.sendMessage(new TextMessage(wsession.getRemoteAddress().getAddress().getHostAddress() +" [" +adminvo.getName()+ "]" + "님이 <span style='color: red;'>퇴장</span>했습니다.")); 
	            }
	        }
